...
public class GenerateAlternativeCodeForParser extends DepthFirstAdapter {
    ...
    public void inAAltTransform(AAltTransform node) {
        ...
        for (int i = 0; i < temp.length; i++) {
            ...
            try {
                ...
                throw new RuntimeException("An error occured while writing to " + new File(pkgDir, "Parser.java").getAbsolutePath());
            }
        }
    }
    ...
    public void inASimpleTerm(ASimpleTerm node) {
        try {
            ...
            macros.apply(file, "ParserSimpleTerm", new String[] { type_name.toLowerCase(), "" + position, type, "" + elemPosition, "" + positionMap });
        }
        catch (IOException e) {
            throw new RuntimeException("An error occured while writing to " + new File(pkgDir, "Parser.java").getAbsolutePath());
        }
    ...
    public void outANewTerm(ANewTerm node) {
        ...
        try {
            ...
            String new_type_name = type_name;
            int new_position = position;
            macros.apply(file, "ParserNewBodyNew", new String[] { new_type_name, "" + new_position, newAltName });
            ...
            macros.apply(file, "ParserNew&Position", new String[] { new_type_name, "" + new_position });
            if (node.getParams().size() > 0) {
                Object temp[] = node.getParams().toArray();
                String isNotTheFirstParam = "";
                ...
                for (int i = 0; i < temp.length; i++) {
                    ...
                    if (!type_name.equals("null")) {
                        if (type_name.startsWith("L")) {
                            type_name = "list";
                            macros.apply(file, "ParserNew&Position&ListParams", new String[] { type_name, "" + position });
                        }
                        else {
                            type_name = type_name.toLowerCase();
                            macros.apply(file, "ParserNew&Position&Params", new String[] { type_name, "" + position });
                        }
                    }
                }
            }
            macros.apply(file, "ParserNewBodyNewTail", new String[] { new_type_name, "" + new_position });
            macros.apply(file, "ParserBraceClosing");
        }
        catch (IOException e) {
            throw new RuntimeException("An error occured while writing to " + new File(pkgDir, "TokenIndex.java").getAbsolutePath());
        }
    }
    ...
    public String name(AProdName node) {
        if (node.getProdNameTail() != null) { return "A" + ResolveIds.name(node.getProdNameTail().getText()) + ResolveIds.name(node.getId().getText()); }
