...
class CEmit
{
  ...
  private void set
	...
	if (CUtility.DEBUG)
	  {
	    CUtility.xassert(null != spec);
	    CUtility.xassert(null != outstream);
	  }
  ...
  /*void emit_imports
      ...
      throws java.io.IOException      
	{
	  ...
	  if (CUtility.DEBUG)
	    {
	      CUtility.xassert(null != m_spec);
	      CUtility.xassert(null != m_outstream);
	    }*/
  ...
  private void emit_table
	...
	for (elem=0; elem<size; elem++) {
	    ...
	    CUtility.xassert(dtrans.m_dtrans.length==m_spec.m_dtrans_ncols);
  ...
  private void emit_actions 
      ...
      throws java.io.IOException
	{
	  ...
	  if (CUtility.DEBUG)
	    {
	      CUtility.xassert(m_spec.m_accept_vector.size() 
...
class CMakeNfa
{
  ...
  private void set
	...
	if (CUtility.DEBUG)
	  {
	    CUtility.xassert(null != input);
	    CUtility.xassert(null != lexGen);
	    CUtility.xassert(null != spec);
	  }
  ...
  void allocate_BOL_EOF
	...
	{
	  CUtility.xassert(CSpec.NUM_PSEUDO==2);
...
class CMinimize
{
  ...
  private void reduce
	...
	for (reduced_ncols = 0; ; ++reduced_ncols)
	  {
		...
		for (i = 0; i < reduced_ncols; ++i)
		  {
		    CUtility.xassert(-1 != m_spec.m_col_map[i]);
		  }
	      }
	    ...
	    if (CUtility.DEBUG)
	      {
		CUtility.xassert(false == set.get(i));
		CUtility.xassert(-1 == m_spec.m_col_map[i]);
	      }
	...
	if (CUtility.DEBUG)
	  {
	    CUtility.xassert(k == reduced_ncols);
	  }
	...
	for (reduced_nrows = 0; ; ++reduced_nrows)
	  {
		...
		for (i = 0; i < reduced_nrows; ++i)
		  {
		    CUtility.xassert(-1 != m_spec.m_row_map[i]);
		  }
	      }
	...
	if (CUtility.DEBUG)
	  {
	    ...
	    CUtility.xassert(k == reduced_nrows);
	  }
      }
	...
	while (old_group_count != group_count)
	  {
	    ...
	    if (CUtility.DEBUG)
	      {
		CUtility.xassert(m_group.size() == group_count);
	      }
		...
		for (j = 1; j < group_size; ++j)
		  {
			...
			if (goto_first != goto_next
				...
				|| m_ingroup[goto_next] != m_ingroup[goto_first]))
			  {
			    if (CUtility.DEBUG)
			      {
				CUtility.xassert(dtrans_group.elementAt(j) == next);
			      }
			    ...
			    if (CUtility.DEBUG)
			      {
				CUtility.xassert(m_group.contains(new_group)
						== true);
				CUtility.xassert(m_group.contains(dtrans_group)
						== true);
				CUtility.xassert(dtrans_group.contains(first)
						== true);
				CUtility.xassert(dtrans_group.contains(next)
						== false);
				CUtility.xassert(new_group.contains(first)
						== false);
				CUtility.xassert(new_group.contains(next)
						== true);
				CUtility.xassert(dtrans_group.size() == group_size);
				CUtility.xassert(i == m_ingroup[first.m_label]);
				CUtility.xassert((m_group.size() - 1) 
	...
	for (i = 0; i < size; ++i)
	  {
	    ...
	    if (CUtility.DEBUG)
	      {
		CUtility.xassert(i == dtrans.m_label);
		CUtility.xassert(false == group_found);
		CUtility.xassert(group_count == m_group.size());
	      }
		...
		if (CUtility.DEBUG)
		  {
		    CUtility.xassert(false == group_found);
		    CUtility.xassert(0 < dtrans_group.size());
		  }
		...
		if (CUtility.SLOW_DEBUG)
		  {
		    ...
		    s = dtrans_group.size();
		    CUtility.xassert(0 < s);

		    for (k = 1; k < s; ++k)
		      {
			check = (CDTrans) dtrans_group.elementAt(k);
			CUtility.xassert(check.m_accept == first.m_accept);
		      }
		  }
		...
		if (first.m_accept == dtrans.m_accept)
		  {
		    ...
		    if (CUtility.DEBUG)
		      {
			CUtility.xassert(j == m_ingroup[dtrans.m_label]);
		      }
...
class CNfa2Dfa
{
	...
	for (istate = 0; nstates > istate; ++istate)
	  {
	    ...
	    */
		...
		if (CUtility.DEBUG)
		  {
		    CUtility.xassert(false == dfa.m_mark);
		  }
		...
		for (i = 0; i < m_spec.m_dtrans_ncols; ++i)
		  {
		    if (CUtility.DEBUG)
		      {
			CUtility.xassert(0 <= i);
			CUtility.xassert(m_spec.m_dtrans_ncols > i);
		      }
		    ...
		    if (CUtility.DEBUG)
		      {
			CUtility.xassert((null == bunch.m_nfa_set 
		    ...
		    if (CUtility.DEBUG)
		      {
			CUtility.xassert(nextstate < m_spec.m_dfa_states.size());
		      }
		...
		if (CUtility.DEBUG)
		  {
		    CUtility.xassert(m_spec.m_dtrans_vector.size() == dfa.m_label);
		  }
  ...
  private void e_closure
	...
	if (CUtility.DEBUG)
	  {
	    CUtility.xassert(null != bunch);
	    CUtility.xassert(null != bunch.m_nfa_set);
	    CUtility.xassert(null != bunch.m_nfa_bit);
	  }
	...
	for (i = 0; i < size; ++i)
	  {
	    ...
	    if (CUtility.DEBUG)
	      {
		CUtility.xassert(bunch.m_nfa_bit.get(state.m_label));
	      }
	...
	while (false == nfa_stack.empty())
	  {
		...
		if (CUtility.DEBUG)
		  {
		    CUtility.xassert(null != bunch.m_accept);
		    CUtility.xassert(CSpec.NONE == bunch.m_anchor
		...
		if (null != state.m_next)
		  {
			...
			if (CUtility.DEBUG)
			  {
			    CUtility.xassert(false == bunch.m_nfa_bit.get(state.m_next.m_label));
			  }
		...
		if (null != state.m_next2)
		  {
			...
			if (CUtility.DEBUG)
			  {
			    CUtility.xassert(false == bunch.m_nfa_bit.get(state.m_next2.m_label));
			  }
...
class CLexGen 
{
  ...
  void generate
	...
	if (CUtility.DEBUG)
	  {
	    CUtility.xassert(null != this);
	    CUtility.xassert(null != m_outstream);
	    CUtility.xassert(null != m_input);
	    CUtility.xassert(null != m_tokens);
	    CUtility.xassert(null != m_spec);
	    CUtility.xassert(m_init_flag);
	  }
  ...
  private char[] packCode
	...
	if (CUtility.DEBUG)
	  {
	    CUtility.xassert(INIT_CODE == specified 
  ...
  private void userRules
	...
	if (CUtility.DEBUG)
	  {
	    CUtility.xassert(END_OF_INPUT == m_spec.m_current_token);
	  }
	...
	for (rep_elem = 0; rep_elem < start_macro; ++rep_elem)
	  {
	    ...
	    if (CUtility.DEBUG)
	      {
		CUtility.xassert(rep_elem < replace.length);
	      }
	  }
  ...
  private void saveMacro
	...
	if (CUtility.DEBUG)
	  {
	    CUtility.xassert(0 < count_def);
	    CUtility.xassert(0 < count_name);
	    CUtility.xassert(null != m_spec.m_macros);
	  }
  ...
  private void saveStates
	...
	if (CUtility.DEBUG)
	  {
	    CUtility.xassert('%' == m_input.m_line[0]);
	    CUtility.xassert('s' == m_input.m_line[1]);
	    CUtility.xassert(m_input.m_line_index <= m_input.m_line_read);
	    CUtility.xassert(0 <= m_input.m_line_index);
	    CUtility.xassert(0 <= m_input.m_line_read);
	  }
  ...
  private void details
	...
	while (names.hasMoreElements())
	  {
	    ...
	    if (CUtility.DEBUG)
	      {
		CUtility.xassert(null != name);
		CUtility.xassert(null != def);
	      }
	...
	if (false == m_spec.m_count_chars)
	...
	else
	  {
	    if (CUtility.DEBUG)
	      {
		CUtility.xassert(m_spec.m_count_lines);
	      }
...
final class SparseBitSet implements Cloneable {
	...
	CUtility.xassert(size<bits.length);
	insert_block(idx, bnum);
    }
    private void insert_block(int idx, int bnum) {
	CUtility.xassert(idx<=size);
	CUtility.xassert(idx==size || offs[idx]!=bnum);
	...
	CUtility.xassert(!a.get(0) && !a.get(1));
	CUtility.xassert(!a.get(123329));
	a.set(0); CUtility.xassert(a.get(0) && !a.get(1));
	a.set(1); CUtility.xassert(a.get(0) && a.get(1));
	a.clearAll();
	CUtility.xassert(!a.get(0) && !a.get(1));
	...
	for (int n=0; n<ITER; n++) {
	    ...
	    CUtility.xassert(a.get(rr) && !a.get(rr+1) && !a.get(rr-1));
	    for (int i=0; i<v.size(); i++)
		CUtility.xassert(a.get(((Integer)v.elementAt(i)).intValue()));
	}
	SparseBitSet b = (SparseBitSet) a.clone();
	CUtility.xassert(a.equals(b) && b.equals(a));
	for (int n=0; n<ITER/2; n++) {
	    ...
	    CUtility.xassert(!b.get(m));
	}
	CUtility.xassert(!a.equals(b));
	SparseBitSet c = (SparseBitSet) a.clone();
	SparseBitSet d = (SparseBitSet) a.clone();
	c.and(a);
	CUtility.xassert(c.equals(a) && a.equals(c));
	c.xor(a);
	CUtility.xassert(!c.equals(a) && c.size()==0);
	d.or(b);
	CUtility.xassert(d.equals(a) && !b.equals(d));
	d.and(b);
	CUtility.xassert(!d.equals(a) && b.equals(d));
	d.xor(a);
	CUtility.xassert(!d.equals(a) && !b.equals(d));
