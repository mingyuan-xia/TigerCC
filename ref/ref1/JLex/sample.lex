...
class Utility {
  public static void xassert
    (
     boolean expr
     )
      { 
	if (false == expr) {
	  throw (new Error("Error: xassertion failed."));
	}
      }
...
<COMMENT> "*/" { 
	...
	Utility.xassert(comment_count >= 0);
	if (comment_count == 0) {
    		yybegin(YYINITIAL);
	}
}
...
<YYINITIAL> \"{STRING_TEXT}\" {
	...
	Utility.xassert(str.length() == yytext().length() - 2);
...
<YYINITIAL> \"{STRING_TEXT} {
	...
	Utility.xassert(str.length() == yytext().length() - 1);
